...
function SubtitleStyleOverrides({ sub, onChange }: { sub: SubtitleEntry; onChange: (patch: Partial<SubtitleEntry>) => void }) {
  const [open, setOpen] = useState(false)
  const hasOverride = sub.color !== undefined || sub.bgColor !== undefined || sub.fontSize !== undefined || sub.bold !== undefined || sub.x !== undefined || sub.y !== undefined
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1 text-[10px] text-gray-400 hover:text-indigo-500 transition-colors mt-0.5"
      >
        <span style={{ fontSize: 8 }}>{open ? '▾' : '▸'}</span>
        Özel Stil {hasOverride && <span className="text-indigo-500 font-bold">●</span>}
      </button>
      {open && (
        <div className="mt-1.5 p-2 bg-gray-50 border border-gray-100 rounded-md space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] text-gray-400 mb-0.5 uppercase font-semibold">Metin Rengi</label>
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  className="flex-1 min-w-0 bg-white border border-gray-200 rounded px-1 py-0.5 text-[10px] font-mono"
                />
                {sub.color !== undefined && (
                  <button type="button" onClick={() => onChange({ color: undefined })} className="text-gray-300 hover:text-red-400 text-[10px]">✕</button>
                )}
              </div>
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 mb-0.5 uppercase font-semibold">Arka Plan</label>
              <div className="flex items-center gap-1">
                <input
                  type="color"
                  value={extractHex(sub.bgColor ?? '#000000')}
                ...
                <input
                  ...
                  className="flex-1 min-w-0 bg-white border border-gray-200 rounded px-1 py-0.5 text-[10px] font-mono"
                />
                {sub.bgColor !== undefined && (
                  <button type="button" onClick={() => onChange({ bgColor: undefined })} className="text-gray-300 hover:text-red-400 text-[10px]">✕</button>
                )}
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] text-gray-400 mb-0.5 uppercase font-semibold">Font Boyutu</label>
              <div className="flex items-center gap-1">
                <input
                  type="number"
                  min={16}
                  ...
                  className="flex-1 bg-white border border-gray-200 rounded px-1 py-0.5 text-[10px]"
                />
                {sub.fontSize !== undefined && (
                  <button type="button" onClick={() => onChange({ fontSize: undefined })} className="text-gray-300 hover:text-red-400 text-[10px]">✕</button>
                )}
              </div>
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 mb-0.5 uppercase font-semibold">Y pozisyon</label>
              <div className="flex items-center gap-1">
                <input
                  type="number"
                  min={0}
                  ...
                  value={sub.y ?? ''}
                  placeholder="global"
                  onChange={e => onChange({ y: e.target.value ? Number(e.target.value) : undefined })}
                  className="flex-1 bg-white border border-gray-200 rounded px-1 py-0.5 text-[10px]"
                />
                {sub.y !== undefined && (
                  <button type="button" onClick={() => onChange({ y: undefined })} className="text-gray-300 hover:text-red-400 text-[10px]">✕</button>
                )}
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] text-gray-400 mb-0.5 uppercase font-semibold">X pozisyon</label>
              <div className="flex items-center gap-1">
                <input
                  type="number"
                  min={0}
                  ...
                  value={sub.x ?? ''}
                  placeholder="global"
                  onChange={e => onChange({ x: e.target.value ? Number(e.target.value) : undefined })}
                  className="flex-1 bg-white border border-gray-200 rounded px-1 py-0.5 text-[10px]"
                />
                {sub.x !== undefined && (
                  <button type="button" onClick={() => onChange({ x: undefined })} className="text-gray-300 hover:text-red-400 text-[10px]">✕</button>
                )}
              </div>
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 mb-0.5 uppercase font-semibold">Kalın</label>
              <select
                value={sub.bold === undefined ? '' : sub.bold ? '1' : '0'}
                onChange={e => onChange({ bold: e.target.value === '' ? undefined : e.target.value === '1' })}
                className="w-full bg-white border border-gray-200 rounded px-1 py-0.5 text-[10px]"
              >
          ...
          {hasOverride && (
            <button
              type="button"
              onClick={() => onChange({ color: undefined, bgColor: undefined, fontSize: undefined, bold: undefined, x: undefined, y: undefined })}
...
export function SubtitleForm({ values, update }: { values: Record<string, unknown>; update: (k: string, v: unknown) => void }) {
  ...
  function patchSubtitle(idx: number, patch: Partial<SubtitleEntry>) {
    const updated = subtitles.map((s, i) => i === idx ? { ...s, ...patch } : s)
    update('subtitles', updated)
  }
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
        {subtitles.map((s, i) => (
          <div key={i} className="border border-gray-200 rounded-lg p-2 bg-white">
            ...
            <SubtitleStyleOverrides sub={s} onChange={patch => patchSubtitle(i, patch)} />
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-gray-100 pt-3 space-y-3">
        ...
        <div>
          ...
          <ColorPicker
            value={extractHex(String(values.subtitleBgColor ?? '#000000'))}
            opacity={100}
            showOpacity={false}
        ...
        <div className="space-y-2">
          <label className="block text-xs text-gray-500 font-medium">Konum (X / Y)</label>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-400 w-4">X</span>
            <input type="range" min={0} max={100} step={1}
              value={Number(values.subtitleX ?? 50)}
              onChange={e => update('subtitleX', Number(e.target.value))}
              className="flex-1" />
            <input type="number" min={0} max={100}
              value={Number(values.subtitleX ?? 50)}
              onChange={e => update('subtitleX', Number(e.target.value))}
              className="w-12 bg-gray-50 border border-gray-200 rounded px-1 py-1 text-xs text-center" />
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-400 w-4">Y</span>
            <input type="range" min={0} max={100} step={1}
              value={Number(values.subtitleY ?? 85)}
              onChange={e => update('subtitleY', Number(e.target.value))}
              className="flex-1" />
            <input type="number" min={0} max={100}
              value={Number(values.subtitleY ?? 85)}
              onChange={e => update('subtitleY', Number(e.target.value))}
          ...
          <p className="text-[10px] text-gray-400">X: 0 sol · 50 orta · 100 sağ &nbsp;|&nbsp; Y: 0 üst · 100 alt</p>
        </div>
