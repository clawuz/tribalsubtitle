import type { VideoCodec, ContainerFormat, ResolutionPreset, FrameRatePreset, ContainerFormatOption, VideoCodecOption } from './types'

export function getCodecString(codec: string): string {
  switch (codec) {
    case 'h264': return 'avc1.4d0028'
    case 'h265': return 'hvc1.1.6.L93.B0'
    case 'vp9':  return 'vp09.00.10.08'
    case 'av1':  return 'av01.0.04M.08'
    default:     return 'avc1.640028'
  }
}

export function isCodecSupportedInContainer(codec: VideoCodec, container: ContainerFormat): boolean {
  if (container === 'webm') return codec === 'vp9' || codec === 'av1'
  return true
}

export function getFallbackCodec(container: ContainerFormat): VideoCodec {
  return container === 'webm' ? 'vp9' : 'h264'
}

export function getVideoCodecsForContainer(container: ContainerFormat): VideoCodecOption[] {
  if (container === 'webm') {
    return [
      { id: 'vp9', label: 'VP9', description: 'Geniş destek' },
      { id: 'av1', label: 'AV1', description: 'En iyi kalite, yavaş' },
    ]
  }
  return [
    { id: 'h264', label: 'H.264 (AVC)', description: 'En uyumlu, hızlı' },
    { id: 'h265', label: 'H.265 (HEVC)', description: 'Daha iyi sıkıştırma' },
    { id: 'vp9',  label: 'VP9', description: 'Açık kaynak codec' },
    { id: 'av1',  label: 'AV1', description: 'En iyi kalite, yavaş' },
  ]
}

export const RESOLUTION_PRESETS: ResolutionPreset[] = [
  { label: '1080p (1920×1080)', width: 1920, height: 1080 },
  { label: '720p (1280×720)',   width: 1280, height: 720 },
  { label: '4K (3840×2160)',   width: 3840, height: 2160 },
  { label: '480p (854×480)',    width: 854,  height: 480 },
]

export const FRAME_RATE_PRESETS: FrameRatePreset[] = [
  { label: '30 fps', fps: 30 },
  { label: '60 fps', fps: 60 },
  { label: '25 fps (PAL)', fps: 25 },
  { label: '24 fps (Film)', fps: 24 },
]

export const CONTAINER_FORMATS: ContainerFormatOption[] = [
  { id: 'mp4',  label: 'MP4',  extension: '.mp4' },
  { id: 'webm', label: 'WebM', extension: '.webm' },
]

export function getRecommendedBitrate(width: number): number {
  if (width >= 3840) return 35_000_000
  if (width >= 1920) return 15_000_000
  if (width >= 1280) return 8_000_000
  return 5_000_000
}

export const BITRATE_RANGE = { min: 1_000_000, max: 100_000_000, step: 500_000 }

export function formatBitrate(bitrate: number): string {
  if (bitrate >= 1_000_000) return `${(bitrate / 1_000_000).toFixed(1)} Mbps`
  return `${(bitrate / 1_000).toFixed(0)} Kbps`
}

export async function checkCodecSupport(codec: VideoCodec, width: number, height: number): Promise<boolean> {
  if (!('VideoEncoder' in window)) return false
  try {
    const support = await VideoEncoder.isConfigSupported({
      codec: getCodecString(codec), width, height, bitrate: 10_000_000, framerate: 30,
    })
    return support.supported ?? false
  } catch { return false }
}
